        ...
        using (System.IO.StreamWriter file = new System.IO.StreamWriter(target_path + scene_name + "_exportedLevelASCII.txt"))
        {
            ...
            foreach (GameObject go in gameObjectsInScene)
            {
                ...
                if(PrefabUtility.GetPrefabAssetType(go) == PrefabAssetType.Regular)
                {
                    ...
                    if (go.GetComponent<Player>())//Player.cs is placeholder
                    {
                        savingObject.myCharacterData = new CharacterData();
                        savingObject.myCharacterData.myName         = go.GetComponent<Player>().myName;
                        savingObject.myCharacterData.myHealth       = go.GetComponent<Player>().myHealth;
                        savingObject.myCharacterData.myDamage       = go.GetComponent<Player>().myDamage;
                        savingObject.myCharacterData.myAttackSpeed  = go.GetComponent<Player>().myAttackSpeed;
                        savingObject.myCharacterData.myMoveSpeed    = go.GetComponent<Player>().myMoveSpeed;
                        savingObject.myCharacterData.myCastSpeed    = go.GetComponent<Player>().myCastSpeed;
                    }
                }
